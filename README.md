...
Работа происходит через обращения к классу `PaymentPageSdk`. SDK поддерживает два основных сценария работы:
- Открытие платежной формы с созданием заказа на стороне плательщика
- Открытие платежной формы с заранее созданным заказом

От сценария зависит передача параметров в конструктор класса и вызываемый метод. Далее разделы будут делиться на эти сценарии.

### `Создание заказа на стороне плательщика`
...
### `Заранее созданный заказ`
Теперь можно открывать форму по уже готовой ссылке, без необходимости передавать 
`publicId`. В этом случае нужно передать в конструктор только обязательный `url` с параметром `payformId`.

Если нужно выбрать определенный сервер для работы (test/production), нужно вставить в `url` ссылку c соответствующим доменом (`pay-test.raif.ru` / `pay.raif.ru`).

Подробнее про метод генерации ссылки можно почитать в [документации API](https://pay.raif.ru/doc/ecom.html#tag/payform/operation/post-payments-v1-merchants-publicId-orders-payment-link2)

```js
    const paymentPage = new PaymentPageSdk({
        url: 'https://pay.raif.ru/pay/?payformId=f96f19db-b53d-4087-b3fa-034aecd682ce'
    });
```
...
* publicId (String) - идентификатор продавца (он уже передан в момент инициализации);
...
### `Заранее созданный заказ`
В этом случае обязательных параметров нет. Из необязательных параметров поддерживается только `style`, который стилизует форму. Если вместе с ним будут переданы другие необязательные параметры, они будут проигнорированы.

Использование самих методов в этом сценарии аналогично предыдущему. Главное отличие – в инициализации класса.
...
#### Форма оплаты в той же вкладке
Для наглядности ниже приведен пример со стилизацией
...
Ниже приведены расширенные сценарии для случая с `созданием заказа на стороне плательщика`. 
Подобные сценарии для случая с открытием формы с `заранее созданным заказом` регулируются на стадии её генерации. Отдельного рассмотрения заслуживает сценарий со стилизацией формы, который описан отдельно в предыдущем разделе.
